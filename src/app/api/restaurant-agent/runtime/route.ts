...
export async function POST(request: NextRequest) {
  ...
  if (body.action === 'claw-experience-default-path') {
    ...
    const providerReceiptAcceptanceConsole = buildRestaurantProviderReceiptAcceptanceConsole({
      providerRunPacket,
      providerReceiptInbox,
      providerReceiptLifecycle,
      now,
    });
    const providerLiveRunGate = buildRestaurantProviderLiveRunGate({
      providerReadinessHealth,
      browserGatewayPack,
      providerSandboxSubmitWorkbench,
      providerRunPacket,
    ...
    const providerLiveRunLaunchAttempt = buildRestaurantProviderLiveRunLaunchAttempt({
      providerLiveRunGate,
      now,
    });
