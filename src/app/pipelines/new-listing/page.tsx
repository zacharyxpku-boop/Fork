...
function NewListingPipelineInner() {
  ...
  const handleExportTables = async () => {
    ...
    const sheetFromText = (name: string, content: string) => ({
      name,
      rows: [['内容'], ...content.split('\n').map(line => [line])],
    });
    ...
    await downloadCsvWorkbook([
      ...
      sheetFromText('翻译', states.translate.result || '(空)'),
      sheetFromText('文案', states.copywriting.result || '(空)'),
      sheetFromText('合规', states['ip-compliance'].result || '(空)'),
    ], exportFilename('新品上新', `${catLabel}-${firstLine}`, 'zip'));
  };
...
`;
  ...
  const handleBatchExport = async () => {
    ...
    ], exportFilename('新品批量', `${cat?.label || ''}-${batchRows.length}条`, 'zip'));
  };
