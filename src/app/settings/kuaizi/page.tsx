...
const EXTERNAL_SETUP_LANES = [
  ...
  {
    title: '门店活动发布',
    wenaiCanDo: '我可以接门店活动账本、预算上限、素材绑定、停止条件和反馈回流。',
    youProvide: '商户授权、平台活动权限、测试预算、测试活动或 sandbox。',
    acceptance: '能读取或提交一条测试活动，并把发布凭证、点击、券领取、预约或私信咨询回流。',
  },
  {
    title: '平台表现回流',
    wenaiCanDo: '我可以接指标字段映射、定时同步、去重、归因窗口和品牌学习回流。',
    youProvide: '平台反馈读取权限、账号 ID、指标定义、同步频率、时区和归因规则。',
...
const CAST_OPERATING_BOARD = [
  {
    ...
    external: '需要平台开发者应用、授权回调白名单、测试账号、店铺/主页/商户 ID。',
    gate: '没有授权就绪或手工就绪的账号，不能把任何发布任务标记为已发布。',
  },
  {
    stage: 'PubPal 矩阵分发',
    ...
    gate: '没有平台发布证据链接前，只能算 handoff，不能算外部发布执行或矩阵分发完成。',
  },
  {
    stage: '门店活动发布',
    goal: '把 Offer Test Matrix 变成真实门店活动：预算、素材、客群、停止条件、券领取/预约/咨询回流。',
    internal: 'Wenai 已有门店活动账本、预算门槛、素材绑定、证据链接、回流门禁和风险检查。',
    external: '需要商户授权、平台活动权限、测试预算、sandbox 或可控测试活动。',
    gate: '没有商户授权、预算和平台活动证据，不能宣称外部发布已完成或自动优化。',
  },
  {
    stage: '表现自动同步',
    goal: '把自然发布、门店活动、预约/券领取和互动指标同步回 Wenai，反哺 Compose / Cut 下一轮生产。',
    internal: 'Wenai 已有 CSV/API 回流入口、字段映射、去重、归因窗口和品牌学习回写。',
    external: '需要平台反馈读取权限、指标定义、同步频率、时区、归因规则和平台 API 配额。',
...
const PROVIDER_MATERIAL_PACKS = [
  {
    ...
    provideVia: '在部署平台配置服务端 secret；测试素材用沙盒项目或无敏感菜品/门店资料。',
  ...
  },
  {
    ...
    unlocks: '解锁平台账号池、授权状态、账号健康、发布频率、矩阵排班和 dispatch 门禁读取。',
    ...
    acceptance: '至少一个平台账号完成授权，账号状态进入授权就绪，并能被分发门禁读取。',
  },
  {
    priority: 'P0',
    pack: '门店活动授权 / 发布回执包',
    why: '决定门店活动是否能从活动账本进入真实预算和素材实验。',
    unlocks: '解锁商户活动读取、测试活动创建、预算门禁、素材绑定、停投规则和活动反馈回流。',
    blockedGate: '缺失时只能记录门店活动假设和手动导入结果，不宣称外部发布已完成或自动优化。',
    provideVia: '平台授权测试商户，先用 sandbox 或小预算白名单活动。',
    materials: ['商户 ID', '活动账号 ID', '创建/读取权限', '测试预算', '券领取/预约事件', '停投规则'],
    doNotSend: '不要开放无限预算或生产账户全权限；先用最小权限和预算上限。',
    acceptance: 'Wenai 能读取或创建测试活动，并把 publish proof、click、coupon claim、reservation 写回反馈回流。',
  },
  {
    priority: 'P1',
    pack: '反馈回流 / 数据导入包',
    ...
    provideVia: '平台反馈 API 或定时导出任务，字段先做只读同步。',
...
const MATERIAL_ACQUISITION_PLAYBOOK = [
  ...
  },
  {
    ...
    proof: '账号进入授权就绪，Wenai 可读账号身份、健康状态、发布频率和可用槽位。',
  },
  {
    lane: '门店活动',
    where: '各平台活动后台或商务管理后台',
    steps: ['创建测试商户活动', '配置最小权限', '设置预算上限', '绑定券领取/预约事件和停投规则'],
    proof: '能读取或创建一条小预算活动，并回传 publish proof、click、coupon claim、reservation。',
  },
  {
    lane: '反馈回流',
    where: '平台反馈 API、活动报表 API 或定时导出任务',
    steps: ['确认指标定义', '配置 attribution window', '绑定 asset_ref/UTM', '设定同步频率'],
    proof: '同步数据能落到 dispatch、门店活动、菜品/套餐和品牌学习档案，失败任务可见。',
  },
  {
    lane: '企业云资产',
  ...
  {
    lane: '规模审计',
    where: 'Wenai 生产账本、平台发布后台、平台表现报表和客户确认材料',
  ...
  return (
    <main className="mx-auto max-w-3xl px-6 py-10">
      ...
      <section className="mt-6 rounded-md border border-slate-200 bg-slate-950 p-6 text-white">
        ...
        <h2 className="mt-2 text-2xl font-black">账号矩阵、PubPal 分发、门店活动发布和反馈回流的统一门禁</h2>
        <p className="mt-2 text-[13px] leading-6 text-white/65">
          Cast 不是把内容“发出去”这么简单，而是把账号池、发布槽位、商户授权、证据链接和反馈回流串成一个可审计闭环。内部账本已经能跑；外部平台授权、发布权限、商户活动权限和反馈回流接齐后，才进入真实平台级执行。
        </p>
      ...
      <section className="mt-6 rounded-md border border-slate-200 bg-white p-6">
        ...
        <p className="mt-2 text-[13px] leading-6 text-slate-600">
          这张清单把“需要外部能力”拆成可执行交付物。P0 先打通真实生成、真实账号和真实门店活动；P1 再补反馈回流、企业云资产和自有规模审计。任何 secret 都只进入服务端环境或部署平台，不进入仓库、不进入浏览器、不进入报告。
        </p>
          ...
          <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
            <div>
              ...
              <p className="mt-1 text-[12px] leading-5 text-slate-600">
                下面只写“去哪里拿”和“拿到后怎么验收”。密钥不贴给我，放进部署平台 secret；账号授权走平台授权或可撤销测试账号。
              </p>
            </div>
        ...
        <div className="mt-5 rounded-md border border-sky-200 bg-sky-50 p-4">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
            <div>
              ...
              <p className="mt-1 text-[12px] leading-5 text-slate-600">
                对齐 /status 的内部 / 外部交付边界板：P0 先让真实生成、真实账号、真实门店活动能跑；P1 再把回流、云资产和规模审计补齐。
              </p>
            </div>
          ...
          <div className="mt-4 grid gap-3 md:grid-cols-4">
            <MaterialGateMetric label="P0 阻断包" value={`${p0MaterialPacks.length} 个`} detail="视频 provider、平台授权、商户活动权限必须先打通。" />
            <MaterialGateMetric label="P1 加厚包" value={`${p1MaterialPacks.length} 个`} detail="反馈回流、云资产、规模审计决定商用厚度。" />
            <MaterialGateMetric label="材料字段" value={`${totalMaterialItems} 项`} detail="每项都只进服务端环境或平台后台，不进聊天和仓库。" />
            <MaterialGateMetric label="放行口径" value="证据优先" detail="没有 callback、发布回执、活动数据或审计账本就保持门禁。" />
          </div>
          <div className="mt-4 grid gap-2 md:grid-cols-3">
            ...
            <div className="rounded-md border border-white bg-white px-3 py-2 text-[12px] leading-5 text-slate-700">
              <span className="font-black text-slate-950">外部提供：</span>你统一给开发者应用、授权账号、provider、商户活动权限、对象存储和审计材料。
            </div>
            <div className="rounded-md border border-white bg-white px-3 py-2 text-[12px] leading-5 text-slate-700">
          ...
          {PROVIDER_MATERIAL_PACKS.map(pack => (
            <article className="rounded-md border border-slate-200 bg-slate-50 p-4" key={pack.pack}>
              ...
              <div className="mt-3 grid gap-2 md:grid-cols-3">
                ...
                <MaterialCheck label="有可验证回执" detail="必须能拿到 callback、授权状态、发布链接、活动指标或审计台账。" />
              </div>
