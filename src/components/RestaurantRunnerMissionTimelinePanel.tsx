  ...
  return (
    <div className="mt-3 border border-amber-200/15 bg-amber-200/[0.035] p-3">
      <div className="flex flex-col gap-2 lg:flex-row lg:items-start lg:justify-between">
        <div>
          <div className="text-[10px] font-semibold uppercase tracking-[0.14em] text-amber-100/65">runner mission timeline</div>
          ...
          <p className="mt-1 text-[11px] leading-4 text-amber-100/55">mission: {timeline?.missionId || 'waiting-for-launch'} / verdict: {timeline?.verdict || 'blocked-needs-owner'}</p>
        </div>
        <div className="border border-white/10 bg-stone-950/45 px-3 py-2 text-right">
          <div className="text-[10px] font-semibold uppercase tracking-[0.14em] text-white/40">open until receipt</div>
      ...
      <div className="mt-3 grid gap-2 md:grid-cols-6">
        <div className="border border-white/10 bg-stone-950/45 p-2">
          <div className="text-[10px] font-semibold uppercase tracking-[0.14em] text-white/40">items</div>
          <div className="mt-1 text-xs font-black text-amber-100/75">{timeline?.summary.timelineItems ?? items.length}</div>
        </div>
        <div className="border border-white/10 bg-stone-950/45 p-2">
          <div className="text-[10px] font-semibold uppercase tracking-[0.14em] text-white/40">live</div>
          <div className="mt-1 text-xs font-black text-amber-100/75">{timeline?.summary.liveEvents ?? 0}</div>
        </div>
        <div className="border border-white/10 bg-stone-950/45 p-2">
          <div className="text-[10px] font-semibold uppercase tracking-[0.14em] text-white/40">planned</div>
        ...
        <div className="border border-white/10 bg-stone-950/45 p-2">
          <div className="text-[10px] font-semibold uppercase tracking-[0.14em] text-white/40">stale</div>
      ...
      <div className="mt-3 grid gap-2 lg:grid-cols-4">
        {items.slice(0, 8).map((item, index) => (
          <div className="border border-white/10 bg-stone-950/45 p-2" key={item.id}>
            ...
            <p className="mt-1 text-[11px] leading-4 text-amber-100/55">owner: {item.owner}</p>
