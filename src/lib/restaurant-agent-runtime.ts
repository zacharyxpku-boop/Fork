...
export const RESTAURANT_AGENT_REFERENCES: RestaurantAgentReference[] = [
  {
    ...
    usefulCapability: '本地浏览器执行、技能、文件/命令动作、常驻本地记忆',
    canAttachNow: false,
    attachRequirement: 'OpenClaw 守护进程或命令行、已审核技能、隔离浏览器环境、限定工作区权限',
    safetyBoundary: '不装未审核技能，没有明确授权不让助手操作商家账号。',
  },
  {
    name: 'Lobu',
    url: 'https://lobu.ai/guides/architecture/',
    usefulCapability: '开源风格的多租户执行层，含网关、执行器、巡检、工具、记忆和密钥代理',
    canAttachNow: false,
    attachRequirement: 'Lobu 执行服务或自部署仓库、连接器 SDK、平台授权配置和租户隔离策略',
    safetyBoundary: '执行器绝不持有第三方原始 token，连接器事件要有审计日志和租户范围。',
  },
  {
    name: 'Hermes',
    url: 'https://docs.browser-use.com/cloud/tutorials/integrations/hermes-agent',
    usefulCapability: '带浏览器控制和外部记忆服务的执行工作流',
    canAttachNow: false,
    attachRequirement: 'Hermes 执行层、浏览器凭据或本地浏览器环境、记忆后端选型',
    safetyBoundary: '浏览器控制必须留在沙箱或店长明确批准的浏览器环境里。',
  },
];
...
export const RESTAURANT_AGENT_CONNECTORS: RestaurantAgentConnector[] = [
  ...
  },
  {
    ...
    status: 'internal-ready',
    canRunNow: true,
    capability: '把发布执行、回执抓取、POS 导入和经营分析拆成商家、运营、技术、合规四类前置步骤，逐项显示缺口、证据和解锁能力。',
  ...
  },
  {
    ...
    status: 'internal-ready',
    canRunNow: true,
    capability: '直接回答发布执行、线索承接、核销、真实经营分析现在是 ready、blocked 还是 forbidden，并列出内部可跑替代动作和外部必补条件。',
  ...
  },
  {
    ...
    auditBoundary: '只处理签名回执、手工导入和脱敏摘要；没有授权时不代发、不写核销、不读取私信或后台明细。',
  },
  {
  ...
  },
  {
    ...
    auditBoundary: '没有授权前不能抓后台、不能代发、不能宣称核销已完成。',
  },
  {
    ...
    status: 'requires-merchant-auth',
    canRunNow: false,
    capability: '发布执行、读取发布回执、评论、私信摘要和社群反馈。',
