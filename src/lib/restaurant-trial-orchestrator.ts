...
export function buildRestaurantTrialOrchestratorPack(input: RestaurantTrialOrchestratorInput = {}): RestaurantTrialOrchestratorPack {
  ...
  return {
    ...
    externalGates: [
      '受控浏览器执行前需要外部通道密钥/试跑通道。',
      '读取平台或发布之前，先拿到店长的平台授权。',
      '凭证被验收前需要签名回调或公开链接/截图。',
      '真实经营分析前需要 POS/核销/会员数据合同。',
      '不读取原始私信、手机号、微信 ID 或订单级 POS 行。',
    ],
    safetyBoundary: '此编排器只组合内部产包、凭证槽和负责人队列。不发布、不联系顾客、不核销券码、不读取私信、不拉取 POS 行、无外部通道密钥、店长授权、回调和数据合同不宣称真实增长。',
  };
}
