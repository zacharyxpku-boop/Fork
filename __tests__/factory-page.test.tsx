...
describe('factory page', () => {
  it('gives restaurant trial users a direct intake workspace with deferred advanced audit', async () => {
    ...
    expect(html).toContain('本地可先准备的 100% 试用闭环');
    expect(html).toContain('生成本地闭环任务');
    expect(html).toContain('本地试跑');
    expect(html).toContain('先手工试跑');
    ...
    expect(html).toContain('手工导入先跑，资料补齐后再交接');
    expect(html).toContain('收银汇总');
    expect(html).not.toContain('POS / 收银');
    expect(html).not.toContain('API key');
    expect(html).not.toContain('Provider');
    expect(html).not.toContain('OpenClaw');
    expect(html).not.toContain('Claw');
    expect(html).toContain('未导入前不判断实际盈亏');
    expect(html).toContain('内容规模先收住');
    expect(html).toContain('默认可操作区');
    expect(html).toContain('高级复核区');
    expect(html).toContain('展开查看打法对标、产品边界和待补资料');
    expect(html).toContain('默认收起');
    expect(html).toContain('高级复核默认不加载到工作台正文');
