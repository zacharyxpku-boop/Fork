...
describe('settings pages', () => {
  ...
  it('renders Kuaizi connection settings without browser-side credential promises', () => {
    ...
    expect(html).toContain('多平台授权 / 账号池');
    expect(html).toContain('外部发布执行 / PubPal 矩阵分发');
    expect(html).toContain('门店活动发布');
    expect(html).toContain('Cast Operating Board');
    expect(html).toContain('账号矩阵、PubPal 分发、门店活动发布和反馈回流的统一门禁');
    expect(html).toContain('账号矩阵池');
    expect(html).toContain('PubPal 矩阵分发');
    expect(html).toContain('表现自动同步');
    expect(html).toContain('没有授权就绪或手工就绪的账号，不能把任何发布任务标记为已发布');
    expect(html).toContain('没有平台发布证据链接前，只能算 handoff');
    expect(html).toContain('没有商户授权、预算和平台活动证据，不能宣称外部发布已完成或自动优化');
    ...
    expect(html).toContain('P0 先打通真实生成、真实账号和真实门店活动');
    ...
    expect(html).toContain('各平台活动后台或商务管理后台');
    expect(html).toContain('对象存储、CDN、企业网盘或云服务 IAM 控制台');
    expect(html).toContain('账号进入授权就绪');
    ...
    expect(html).toContain('解锁平台账号池、授权状态、账号健康');
    expect(html).toContain('不把任何内容标记为真实发布');
    expect(html).toContain('解锁商户活动读取、测试活动创建');
    expect(html).toContain('不宣称外部发布已完成或自动优化');
    ...
    expect(html).toContain('没有 callback、发布回执、活动数据或审计账本就保持门禁');
    expect(html).toContain('视频生成 / 剪辑 provider 包');
    expect(html).toContain('平台授权 / 账号池授权包');
    expect(html).toContain('门店活动授权 / 发布回执包');
    expect(html).toContain('反馈回流 / 数据导入包');
  ...
  it('keeps the partner-facing external integration material handoff explicit', () => {
    ...
    expect(doc).toContain('Restaurant identity');
    expect(doc).toContain('Channel accounts');
    expect(doc).toContain('Publish proof');
    expect(doc).toContain('Recovery summaries');
    expect(doc).toContain('Voice/front desk');
    expect(doc).toContain('Cost/inventory');
    expect(doc).toContain('Video provider');
    expect(doc).toContain('Scale audit');
    expect(doc).toContain('Restaurant Trial Intake Pack');
    expect(doc).toContain('Restaurant Channel Authorization Pack');
    expect(doc).toContain('Recovery Summary Pack');
    expect(doc).toContain('Voice / Front-Desk Pack');
    expect(doc).toContain('Cost / Inventory Pack');
    expect(doc).toContain('Video Provider Pack');
    expect(doc).toContain('Kuaizi scale numbers such as `91M+ creative output` and `42M+ video distribution` are competitor benchmarks only');
    expect(doc).toContain('No provider callback: do not claim one-click finished video, batch smart remixing or complete video production automation.');
    expect(doc).toContain('No platform authorization: do not claim automatic publishing to Dianping/Meituan, Xiaohongshu, Douyin or WeChat groups.');
    expect(doc).toContain('No POS/redemption/member data contract: do not claim true redemption, repeat purchase, margin, inventory optimization or operating attribution.');
    expect(doc).toContain('No private data storage: do not store phone numbers, WeChat IDs, private-message bodies, coupon codes, order details, raw POS rows, cookies, tokens or API keys.');
    expect(doc).not.toContain('Amazon');
    expect(doc).not.toContain('Shopify');
    expect(doc).not.toContain('Ad Account / Campaign Pack');
